...
def create_personal(nombres, apellidos, documento_identidad, telefono, formacion, evaluacion_periodica):
    conn = get_connection()
    try:
        cursor = conn.cursor()
        sql = """INSERT INTO personal_seguridad 
                 ...
                 VALUES (%s, %s, %s, %s, %s, %s)"""
        cursor.execute(sql, (nombres, apellidos, documento_identidad, telefono, formacion, evaluacion_periodica))
        conn.commit()
